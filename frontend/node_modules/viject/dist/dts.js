...
export const overWriteDTS = () => {
    if (existsSync(appTypeDeclarations)) {
        try {
            const appContent = readFileSync(appTypeDeclarations, "utf8").replace(/^\s*\/\/\/\s*<reference\s+types.+?"react-scripts".*\/>.*(?:\n|$)/gm, "");
            const ownContent = readFileSync(ownTypeDeclarations, "utf8");
            writeFileSync(appTypeDeclarations, ownContent.trim() + EOL + appContent.trim() + EOL);
        }
        catch (e) {
            console.error(e);
